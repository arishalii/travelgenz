...
const DubManagement = () => {
  const [destinations, setDestinations] = useState<DubDestination[]>([]);
  const [editingItem, setEditingItem] = useState<DubDestination | null>(null);
  ...
  const deleteItem = async (id: string) => {
    if (confirm('Are you sure you want to delete this DUB destination?')) {
      await supabase.from('dub_destinations').delete().eq('id', id);
      loadDestinations();
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold">DUB Destinations Management</h3>
        <button
      ...
      <SpreadsheetUpload
        ...
        title="Upload DUB Destinations CSV"
        templateData={destinations}
      />

      {showAddForm && (
        <div className="bg-gray-50 p-4 rounded-lg">
          <h4 className="font-medium mb-4">Add New DUB Destination</h4>
