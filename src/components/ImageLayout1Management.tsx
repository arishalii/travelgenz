...
const ImageLayout1Management = () => {
  const [destinations, setDestinations] = useState<ImageLayout1Destination[]>([]);
  const [drafts, setDrafts] = useState<Draft[]>([]);
  const [editingItem, setEditingItem] = useState<ImageLayout1Destination | null>(null);
  ...
  const loadDrafts = async () => {
    const { data, error } = await supabase
      .from('drafts')
      .select('*')
      .eq('content_type', 'image_layout1_destinations')
  ...
  const handleCSVDataParsed = async (data: any[]) => {
    try {
      console.log('Processing CSV data for image layout1 destinations:', data);
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (error) {
          console.error('Error inserting image layout1 destination:', error);
      ...
      alert(`Successfully uploaded ${data.length} image layout1 destinations!`);
  ...
  const handleSaveDraft = async (item: any) => {
    const { error } = await supabase.from('drafts').insert({
      content_type: 'image_layout1_destinations',
      content_data: item,
  ...
  const deleteItem = async (id: string) => {
    if (confirm('Are you sure you want to delete this image layout1 destination?')) {
      await supabase.from('image_layout1_destinations').delete().eq('id', id);
      loadDestinations();
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold">Image Layout1 Management</h3>
      ...
      {showCSVUpload && (
        <SpreadsheetUpload
          ...
          title="Image Layout1 Destinations CSV Upload"
      ...
      {showAddForm && (
        <div className="bg-gray-50 p-4 rounded-lg">
          <h4 className="font-medium mb-4">Add New Image Layout1 Destination</h4>
...
export default ImageLayout1Management;
