...
import SpreadsheetUploadVisaImage from './SpreadsheetUploadVisaImage';

interface BudgetFriendlyDestination {
  id: string;
  title_line: string;
  name: string;
  image: string;
...
const BudgetFriendlyDestinationsManagement = () => {
  const [destinations, setDestinations] = useState<BudgetFriendlyDestination[]>([]);
  const [drafts, setDrafts] = useState<Draft[]>([]);
  const [editingItem, setEditingItem] = useState<BudgetFriendlyDestination | null>(null);
  ...
  const loadDestinations = async () => {
    const { data, error } = await supabase
      .from('budget_friendly_destinations')
      .select('*')
  ...
  const loadDrafts = async () => {
    const { data, error } = await supabase
      .from('drafts')
      .select('*')
      .eq('content_type', 'budget_friendly_destinations')
  ...
  const handleCSVDataParsed = async (data: any[]) => {
    try {
      console.log('Processing CSV data for budget friendly destinations:', data);
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (error) {
          console.error('Error inserting budget friendly destination:', error);
      ...
      alert(`Successfully uploaded ${data.length} budget friendly destinations!`);
  ...
  const handleSaveDraft = async (item: any) => {
    const { error } = await supabase.from('drafts').insert({
      content_type: 'budget_friendly_destinations',
      content_data: item,
  ...
  const deleteItem = async (id: string) => {
    if (confirm('Are you sure you want to delete this budget friendly destination?')) {
      await supabase.from('budget_friendly_destinations').delete().eq('id', id);
      loadDestinations();
    }
  };
  ...
  const deleteSelectedItems = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.length} selected items?`)) {
      await supabase.from('budget_friendly_destinations').delete().in('id', selectedItems);
      setSelectedItems([]);
      loadDestinations();
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold">Budget Friendly Destinations Management</h3>
      ...
      {showCSVUpload && (
        <SpreadsheetUploadVisaImage
          onDataParsed={handleCSVDataParsed}
          
          title="Budget Friendly Destinations CSV Upload"
      ...
      {showAddForm && (
        <div className="bg-gray-50 p-4 rounded-lg">
          <h4 className="font-medium mb-4">Add New Budget Friendly Destination</h4>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            ...
            <input
              type="text"
              placeholder="Price (e.g., Starting from $499)"
              value={newItem.price}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-travel-primary font-medium">{destination.price}</p>
...
export default BudgetFriendlyDestinationsManagement;