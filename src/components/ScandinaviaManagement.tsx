...
interface ScandinaviaDestination {
  id: string;
...
const ScandinaviaManagement = () => {
  const [destinations, setDestinations] = useState<ScandinaviaDestination[]>([]);
  const [drafts, setDrafts] = useState<Draft[]>([]);
  const [editingItem, setEditingItem] = useState<ScandinaviaDestination | null>(null);
  ...
  const loadDrafts = async () => {
    const { data, error } = await supabase
      .from('drafts')
      .select('*')
      .eq('content_type', 'scandinavia_destinations')
  ...
  const handleSaveDraft = async (item: any) => {
    const { error } = await supabase.from('drafts').insert({
      content_type: 'scandinavia_destinations',
      content_data: item,
  ...
  const deleteItem = async (id: string) => {
    if (confirm('Are you sure you want to delete this Scandinavia destination?')) {
      await supabase.from('scandinavia_destinations').delete().eq('id', id);
      loadDestinations();
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold">Scandinavia Destinations Management</h3>
      ...
      <SpreadsheetUploadUnique
        ...
        title="Upload Scandinavia Destinations CSV"
      ...
      {showAddForm && (
        <div className="bg-gray-50 p-4 rounded-lg">
          <h4 className="font-medium mb-4">Add New Scandinavia Destination</h4>
...
export default ScandinaviaManagement;