...
const DubsssManagement = () => {
  const [destinations, setDestinations] = useState<DubsssDestination[]>([]);
  const [drafts, setDrafts] = useState<Draft[]>([]);
  const [editingItem, setEditingItem] = useState<DubsssDestination | null>(null);
  ...
  const loadDrafts = async () => {
    const { data, error } = await supabase
      .from('drafts')
      .select('*')
      .eq('content_type', 'dubsss_destinations')
  ...
  const handleSaveDraft = async (item: any) => {
    const { error } = await supabase.from('drafts').insert({
      content_type: 'dubsss_destinations',
      content_data: item,
  ...
  const deleteItem = async (id: string) => {
    if (confirm('Are you sure you want to delete this DUBSSS destination?')) {
      await supabase.from('dubsss_destinations').delete().eq('id', id);
      loadDestinations();
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold">DUBSSS Destinations Management</h3>
      ...
      <SpreadsheetUpload
        ...
        title="Upload DUBSSS Destinations CSV"
      ...
      {showAddForm && (
        <div className="bg-gray-50 p-4 rounded-lg">
          <h4 className="font-medium mb-4">Add New DUBSSS Destination</h4>
...
export default DubsssManagement;
