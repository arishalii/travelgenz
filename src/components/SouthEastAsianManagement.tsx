...
const SouthEastAsianManagement = () => {
  const [destinations, setDestinations] = useState<SouthEastAsianDestination[]>([]);
  const [drafts, setDrafts] = useState<Draft[]>([]);
  const [editingItem, setEditingItem] = useState<SouthEastAsianDestination | null>(null);
  ...
  const loadDrafts = async () => {
    const { data, error } = await supabase
      .from('drafts')
      .select('*')
      .eq('content_type', 'south_east_asian_destinations')
  ...
  const handleSaveDraft = async (item: any) => {
    const { error } = await supabase.from('drafts').insert({
      content_type: 'south_east_asian_destinations',
      content_data: item,
  ...
  const deleteItem = async (id: string) => {
    if (confirm('Are you sure you want to delete this South East Asian destination?')) {
      await supabase.from('south_east_asian_destinations').delete().eq('id', id);
      loadDestinations();
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold">South East Asian Destinations Management</h3>
      ...
      <SpreadsheetUploadUnique
        ...
        title="Upload South East Asian Destinations CSV"
      ...
      {showAddForm && (
        <div className="bg-gray-50 p-4 rounded-lg">
          <h4 className="font-medium mb-4">Add New South East Asian Destination</h4>
...
export default SouthEastAsianManagement;