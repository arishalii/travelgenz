...
const OceaniaManagement = () => {
  const [destinations, setDestinations] = useState<OceaniaDestination[]>([]);
  const [drafts, setDrafts] = useState<Draft[]>([]);
  const [editingItem, setEditingItem] = useState<OceaniaDestination | null>(null);
  ...
  const loadDrafts = async () => {
    const { data, error } = await supabase
      .from('drafts')
      .select('*')
      .eq('content_type', 'oceania_destinations')
  ...
  const handleSaveDraft = async (item: any) => {
    const { error } = await supabase.from('drafts').insert({
      content_type: 'oceania_destinations',
      content_data: item,
  ...
  const deleteItem = async (id: string) => {
    if (confirm('Are you sure you want to delete this Oceania destination?')) {
      await supabase.from('oceania_destinations').delete().eq('id', id);
      loadDestinations();
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold">Oceania Destinations Management</h3>
      ...
      <SpreadsheetUploadUnique
        ...
        title="Upload Oceania Destinations CSV"
...
export default OceaniaManagement;