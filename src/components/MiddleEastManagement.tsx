...
const MiddleEastManagement = () => {
  const [destinations, setDestinations] = useState<MiddleEastDestination[]>([]);
  const [drafts, setDrafts] = useState<Draft[]>([]);
  const [editingItem, setEditingItem] = useState<MiddleEastDestination | null>(null);
  ...
  const loadDrafts = async () => {
    const { data, error } = await supabase
      .from('drafts')
      .select('*')
      .eq('content_type', 'middle_east_destinations')
  ...
  const handleSaveDraft = async (item: any) => {
    const { error } = await supabase.from('drafts').insert({
      content_type: 'middle_east_destinations',
      content_data: item,
  ...
  const deleteItem = async (id: string) => {
    if (confirm('Are you sure you want to delete this Middle East destination?')) {
      await supabase.from('middle_east_destinations').delete().eq('id', id);
      loadDestinations();
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold">Middle East Destinations Management</h3>
      ...
      <SpreadsheetUploadUnique
        ...
        title="Upload Middle East Destinations CSV"
      ...
      {showAddForm && (
        <div className="bg-gray-50 p-4 rounded-lg">
          <h4 className="font-medium mb-4">Add New Middle East Destination</h4>
...
export default MiddleEastManagement;