...
const UniqueExperiencesManagement = () => {
  const [experiences, setExperiences] = useState<UniqueExperience[]>([]);
  const [drafts, setDrafts] = useState<Draft[]>([]);
  const [editingItem, setEditingItem] = useState<UniqueExperience | null>(null);
  ...
  const loadDrafts = async () => {
    const { data } = await supabase
      .from('drafts')
      .select('*')
      .eq('content_type', 'unique_experiences')
  ...
  const handleBulkDelete = async () => {
    if (selectedExperiences.length === 0) {
      alert('No experiences selected');
      return;
    }

    if (!confirm(`Are you sure you want to delete ${selectedExperiences.length} experience(s)?`)) return;
    ...
    try {
      ...
      alert(`${selectedExperiences.length} experience(s) deleted successfully`);
      setSelectedExperiences([]);
      loadExperiences();
    } catch (error) {
      console.error('Error deleting experiences:', error);
      alert('Failed to delete experiences');
    }
  };

  const handleSaveDraft = async (item: any) => {
    const { error } = await supabase.from('drafts').insert({
      content_type: 'unique_experiences',
      content_data: item,
  ...
  const handleUpdatePage = async (item: any) => {
    if (item.id) {
      ...
      await supabase
        .from('unique_experiences')
        .insert({ ...item, status: 'published', position: experiences.length + 1 });
    }
    loadExperiences();
    setEditingItem(null);
  ...
  const moveItem = async (id: string, direction: 'up' | 'down') => {
    const currentIndex = experiences.findIndex(d => d.id === id);
    if (
  ...
  const deleteItem = async (id: string) => {
    if (confirm('Are you sure you want to delete this unique experience?')) {
      await supabase.from('unique_experiences').delete().eq('id', id);
      loadExperiences();
    }
  };
  ...
  const publishFromDraft = async (draft: Draft) => {
    if (draft.original_id) {
      ...
      await supabase
        .from('unique_experiences')
        .insert({ ...draft.content_data, status: 'published', position: experiences.length + 1 });
    }
  ...
  const handleSpreadsheetData = async (data: any[]) => {
    setLoading(true);
    try {
      const experiencesData = data.map((row, index) => ({
        ...
        status: 'published',
        position: experiences.length + index + 1
      }));
      ...
      alert(`${experiencesData.length} experiences imported successfully`);
      loadExperiences();
    } catch (error) {
      console.error('Error importing experiences:', error);
      alert('Failed to import experiences');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold">Unique Experiences Management</h3>
      ...
      <SpreadsheetUploadUnique
        ...
        title="Upload Unique Experiences CSV"
      ...
      {showAddForm && (
        <div className="bg-gray-50 p-4 rounded-lg">
          <h4 className="font-medium mb-4">Add New Unique Experience</h4>
            ...
            {editingItem?.id === experience.id ? (
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    {experience.title_line && <p className="text-sm text-gray-600">{experience.title_line}</p>}
                    <span className="font-medium">{experience.name}</span>
                    {experience.discount && <span className="ml-2 text-sm text-orange-600 font-medium">{experience.discount}</span>}
                    <p className="text-sm text-gray-500">Position: {experience.position} | Status: {experience.status || 'published'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <select
                    value={experiences.findIndex(d => d.id === experience.id) + 1}
                    onChange={(e) => updatePosition(experience.id, parseInt(e.target.value))}
                  ...
                  <button
                    onClick={() => toggleStatus(experience.id, experience.status || 'published')}
                    className={`p-1 ${(experience.status || 'published') === 'published' ? 'text-green-500' : 'text-red-500'} hover:opacity-70`}
                    title={(experience.status || 'published') === 'published' ? 'Hold (Hide from website)' : 'Publish (Show on website)'}
                  >
                    {(experience.status || 'published') === 'published' ? <Eye size={16} /> : <EyeOff size={16} />}
                  </button>
                  <button
                    onClick={() => moveItem(experience.id, 'up')}
                    className="p-1 text-gray-500 hover:text-gray-700"
                  >
                    <ArrowUp size={16} />
                  </button>
                  <button
                    onClick={() => moveItem(experience.id, 'down')}
                    className="p-1 text-gray-500 hover:text-gray-700"
                  >
                    <ArrowDown size={16} />
                  </button>
                  ...
                  <button
                    onClick={() => deleteItem(experience.id)}
...
export default UniqueExperiencesManagement;